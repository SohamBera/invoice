...
def move_to_processed(file_id: str):
    try:
        info = drive_service.files().get(fileId=file_id, fields="parents").execute()
        prev = ",".join(info.get("parents", []))
        drive_service.files().update(fileId=file_id, addParents=PROCESSED_FOLDER_ID, removeParents=prev, fields="id,parents").execute()
    except Exception as e:
        print("Move to processed failed:", e)
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt < MAX_RETRIES:
                ...
                print("Google Sheet append permanently failed for this row.")
